...
namespace
{
	...
	inline void push_indices(std::vector<GLushort>& indices, int sectors, int r,
							 int s)
	{
	...
	void createSphere(std::vector<glm::vec3>& vertices,
					  std::vector<GLushort>& indices,
					  std::vector<glm::vec3>& normals,
					  std::vector<glm::vec2>& texcoords, float radius,
...
SphereMesh::SphereMesh(float radius, unsigned int rings, unsigned int sectors)
{
	...
	glBufferData(GL_ARRAY_BUFFER, normals.size() * sizeof(GLfloat) * 3,
...
{
	glBindBuffer(GL_ARRAY_BUFFER, m_vertexBuffer);
	glVertexAttribPointer(RenderConstants::PositionAttributeLocation, 3, GL_FLOAT,
						  0, 0, 0);
	glEnableVertexAttribArray(RenderConstants::PositionAttributeLocation);

	glBindBuffer(GL_ARRAY_BUFFER, m_colorBuffer);
	glVertexAttribPointer(RenderConstants::NormalAttributeLocation, 3, GL_FLOAT,
						  0, 0, 0);
